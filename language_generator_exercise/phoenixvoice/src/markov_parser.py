...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Build a Markov model and generate text')
    parser.add_argument('--order', type=int, default=1,
                        help='Order of the Markov model (default: 1)')
    parser.add_argument('--samples', type=int, default=3,
                        help='Number of text samples to generate (default: 3)')
    parser.add_argument('--max-tokens', type=int, default=50,
                        help='Maximum tokens per sample (default: 50)')
    parser.add_argument('--file', default='chelseatroy_blog_posts.txt',
                        help='Blog posts file (default: chelseatroy_blog_posts.txt)')
    parser.add_argument('--save', type=str, default=None,
                        help='Save the trained model to this file')
    parser.add_argument('--load', type=str, default=None,
                        help='Load a pre-trained model from this file')

    args = parser.parse_args()

    # Load or build model
    if args.load:
        model = MarkovModel.load(args.load)
    else:
        model = build_markov_model_from_file(args.file, order=args.order)
    ...
    for i in range(args.samples):
        text = model.generate_text(max_tokens=args.max_tokens)
        print(f"\nSample {i+1}:")
        print(text)
        print()
